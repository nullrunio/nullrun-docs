...
### Mechanism
The catalog is a docs reference; the enforcement lives in `backend/src/proxy/http/gate/orchestrator.rs::check_tool_block` (line 2522+). The `glob_match` helper (line 2320) implements the pattern language: `*` for wildcards (multi-`*` patterns split on every `*` and require each non-empty literal segment to appear in order via `glob_match_multi` at line 2397), `|` for alternation (line 2324), bare literals match exactly (no substring), `?` is treated as literal. Pattern validation is in `backend/src/proxy/http/tool_canonical.rs::validate_tool_pattern`; `MAX_POLICY_PATTERN_BYTES = 4096` (line 32). Tool names are canonicalised via `classify_tool` (`tool_canonical.rs`) — Builtin (lowercase ASCII ≤64 chars), `mcp://{server}/{tool}` for MCP, `custom:{name}` for custom; invalid format is rejected at gate validation as `VALIDATION_FAILED`.

### Guarantees
- **ToolBlock is always Hard**: per CLAUDE.md §8, soft mode does not soften tool blocks. `tool_blocked()` in `backend/src/proxy/http/gate/internal.rs` is a defensive backstop that also runs on `/track`, so an SDK that bypasses `/check` (or omits `tools` from `/check`) cannot skip tool_pattern enforcement on the wire.
- **Fail-CLOSED on missing tools field**: if the per-key policy has `tool_patterns` and the SDK omits `tools`, the gate blocks with `TB-1` reason code (`orchestrator.rs`); the singular `tool` field is also resolved into `effective_tools` so legacy SDKs cannot bypass by sending the pre-T4 wire shape.
- **Canonical form**: all Builtin names are lowercase ASCII; the gate's classification is the single source of truth (`tool_canonical.rs` comment).
- **Glob match is case-SENSITIVE on the function level** (`gate/internal.rs` test `glob_match_case_sensitive`); Builtin names are lowercased at classification so `Stripe.Charge` → `stripe.charge` before the match — this is what the docs mean by "case-insensitively" for the operator's experience.
- **Glob match does NOT inspect tool arguments** — sandbox responsibility (`tool_canonical.rs`); a pattern like `bash.*` blocks any bash call regardless of the shell command.
...
- **Dotted-prefix smart match**: `bash.*` matches `bash`, `bash.foo`, `bash.foo.bar` — bare `bash` AND dotted continuations (`orchestrator.rs`). Templates seed `bash.*`, `shell.*`, `code.*` patterns; operators expect them to catch the bare name.
...
- **No double-`*` support**: `**` is treated literally per `glob_match_no_double_star_support` (`gate/internal.rs`).
