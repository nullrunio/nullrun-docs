...
- **Plan-tier gate is server-side**: `Feature::Alerts` is enforced on LIST, dismiss, snooze, and dismiss-all handlers (`alerts.rs:154-162`); a Lite user cannot enumerate alerts by API bypass even though the dashboard link is hidden.
...
- **Category invariant**: System alerts MUST have `workflow_id = None` and `workflow_name = None`; the writer enforces before insert (`models.rs:146-148`).
...
### Patterns
- **Three-tier active set**: `dismissed = false AND (snoozed_until IS NULL OR snoozed_until < now())` — the same predicate is reused by the three `count_active_by_severity` calls, so the `info_count` field is NOT a new query (`alerts.rs:192-215`).
