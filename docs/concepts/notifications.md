...
- **HMAC webhook integrity**: outgoing webhooks carry `X-NullRun-Signature`, `X-NullRun-Timestamp`, `X-NullRun-Nonce`; the canonical signing string is `<unix_ts>.<nonce>.<body>` and `HMAC-SHA256(secret, payload)` is computed in constant time (`backend/src/alert/webhook_signing.rs`).
...
- **Fail-CLOSED on notification_settings lookup**: a DB transport error opts the org out (silent opt-out) rather than amplifying during a DB blip (`backend/src/alert/bridges.rs`); the `record_alert_skipped_db_unavailable` counter surfaces the degraded mode.
...
- **Bridge dispatch is fire-and-forget**: bridge dispatchers `tokio::spawn` so the calling site stays synchronous — failures are best-effort and never block the producer (`backend/src/alert/bridges.rs`).
